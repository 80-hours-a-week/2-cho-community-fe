...
- **03-17: 위키 FAQ/지식베이스 UI**
  - 위키 목록/상세/작성/편집 4개 페이지 추가 (26개 엔트리포인트)
  - 태그 필터, 마크다운 에디터, 슬러그 기반 URL

- **03-16: 패키지 리뷰 페이지**
  - 패키지 목록: 카테고리 필터, 검색, 정렬(최신/평점/리뷰수/이름), 무한 스크롤
  - 패키지 상세: 패키지 정보 + 리뷰 목록 + 리뷰 작성/수정/삭제
  - 패키지 등록: 카테고리, 패키지 매니저 선택
...
- **03-16: 알림 유형별 설정 UI**
  - 알림 페이지에 설정 버튼 + 접이식 패널 (5개 타입별 체크박스)
  - 체크박스 변경 시 즉시 API 저장, 실패 시 롤백

- **03-16: 게시글 검색 자동완성**
  - 메인 페이지 검색바에 300ms 디바운스 드롭다운 (최대 5개 결과, 제목+작성자)

- **03-15: E2E 테스트 보강**
  - 댓글 정렬 버튼 3개 표시, active 전환, 수정됨 배지, 이용약관 토글 테스트 추가 (4 tests)

- **03-15: 멘션 프로필 링크 + 모바일 CSS**
  - `@닉네임` 클릭 시 사용자 검색 → 프로필 페이지 이동 (동적 import, 비로그인 시 무시)
  - mention-highlight에 cursor: pointer + hover underline
  - 회원가입 컨테이너 모바일 패딩, 댓글 정렬 바 flex-wrap

- **03-15: 에디터 이미지 업로드 키 수정 + 리스너 누수 해결**
  - `onImageUpload`: `response.data.image_url` → `response.data?.data?.url` (드래그앤드롭/붙여넣기 이미지 삽입 버그)
  - 댓글 정렬 버튼 리스너 누적 방지 (`_sortBound` 플래그)
  - PostFormView, CommentController에 `destroy()` 메서드 추가 + pagehide cleanup

- **03-15: 멘션 UX 보완, 이용약관 UI, 에러 처리, 댓글 인기순**
  - 멘션 정규식 `@(\S+)` → `@([a-zA-Z0-9_]{3,10})` — BE 닉네임 규칙과 동기화
  - 게시글 에디터에 MentionDropdown 연결 (Write/Edit)
  - 게시글 본문에 멘션 하이라이트 적용
  - 댓글 "(수정됨)" 배지 추가 (`updated_at !== created_at` 시)
  - 회원가입 이용약관 동의 체크박스 + 약관 링크
  - 카테고리/댓글/DM 로드 실패 시 토스트 메시지
  - 댓글 정렬 바에 '인기순' 버튼 추가

- **03-12: 차단 목록 관리 UI + 투표 변경/취소 UI + 팔로워·팔로잉 목록 모달**
  - 내 활동 페이지에 "차단 목록" 5번째 탭 추가 — 차단된 사용자 카드(아바타, 닉네임, 날짜) + 차단 해제 버튼(낙관적 카드 제거)
  - 투표 결과 화면에 "투표 변경" / "투표 취소" 버튼 (미만료 + 본인 투표 시만 표시)
  - 투표 변경: 2단계 UX — 결과 → 라디오 폼(버튼 텍스트 "변경") → PUT 제출
  - 투표 취소: DELETE 호출 → 투표 폼으로 복귀
  - 사용자 프로필 팔로워/팔로잉 통계 클릭 → 목록 모달 표시 (닫기 버튼)
  - E2E 테스트: 차단 목록 5개 + 투표 변경/취소 3개 + 팔로워/팔로잉 모달 3개 추가 (총 90개)
...
- **03-10: DM 기능 전면 개선**
  - 데스크톱 좌우 분할 레이아웃 (`dm.html`, 768px 이상 자동 전환)
  - 메신저 앱 수준 UI 리디자인 (라이트/다크 모드 13개 DM 토큰)
  - 날짜 구분선, 읽음 체크마크 (✓/✓✓), 타이핑 인디케이터
  - 메시지 삭제 (우클릭 컨텍스트 메뉴), 위로 스크롤 이전 메시지 로드
  - 대화 목록 실시간 갱신 (WebSocket) + 닉네임 검색
  - DMListController `totalCount` 경로 버그 수정
